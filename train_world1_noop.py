"""World 1 混训 · 带 no-op starts：验证"老师本身在抖动下重训能不能变强"。
...
用法: MARIO_NOOP=30 python train_world1_noop.py [步数] [并行环境数]
"""
...
from make_env import make_env_world1, NOOP_JITTER
...
def main():
    ...
    print(f">>> World1 抖动重训 {total:,} 步 | {n_envs} 环境 | 续 {BASE_MODEL} | "
          f"开局随机空按 0-{NOOP_JITTER} 帧 | ent_coef {model.ent_coef} | device {DEVICE}", flush=True)

    ckpt = CheckpointCallback(save_freq=max(500_000 // n_envs, 1),
    ...
    print(f">>> World1 抖动重训结束，存为 {OUT}.zip", flush=True)
