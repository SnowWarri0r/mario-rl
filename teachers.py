...
TEACHERS = {
    ...
    #        试过的：裸奖励续训 16M（3→13）、再续 20M（13→13，累计 ~36M 纹丝不动）、
    #        塑形（死一次重罚 80 + 沿途六个 checkpoint，13→23）。
    #        塑形是有效的、方向也对，但 23% 当不了老师，且逐档在 1-7/31 之间震荡。
    #        skip=2（水下动作粒度）**已经测了，是负结果**：两个水关各跑 24M 步，
    #        7-2 只到 13%（skip=4 是 23%）、2-2 只到 48%（skip=4 是 84%），双双不如 skip=4。
    #        2-2 在 8M 处见顶、后面 16M 再没起色，不是"还没学完"。
    #        ⚠️ 有个没排除干净的混淆：两臂都是从 skip=4 训出来的策略 warm-start 的，
    #        动作语义变了等于把学到的时机全打乱（2-2 一上来从 84% 摔到 26%）。
    #        真正干净的对照是从零训 skip=2，但按现有趋势不值得再花这个预算。
    #        ⇒ **结论：水关这条短板跟动作粒度无关，"一个网络打完全部关卡"的立意不用动。**
    "7-1": ("checkpoints_mario_71c/mario_71c_9499392_steps.zip",      90, None),
    "7-2": ("checkpoints_mario_72s/mario_72s_9749376_steps.zip",      23, None),  # 见下方水关注记
...
# 还没有老师的关：三个迷宫城堡。**结案，29/32 是最终成绩。**
#
# 正确走法是查得到的（mariowiki / strategywiki）：
#   4-4 两段迷宫＝上 → 下；7-4 ＝ 下 → 中 → 上；8-4 要下管道（仅它需要 MARIO_COMPLEX=1）。
# 也就是说**缺的从来不是答案**。4-4 上前后试过十二种办法，全部 0/31：
#   奖励侧：max-x 势能 / 回卷即终局（唯一见效的一次，过了第一段岔口）/ 回合内 (x,y) novelty /
#           跨回合访问计数 / 按正解编的 (x,y) 路标（闸门处）/ 路标前移到爬升段之前
#   搜索侧：纯随机 / 816 组受控扰动 / 束搜索 7 动作 / 束搜索 12 动作 / 走廊高度扫描
#   Go-Explore：地基没验通（存档开局稳定低 20-30pp，三个假设全证伪）
#
# **最有价值的否定结论**：闸门处的位置不是判据。
#   ① 走廊高度扫描：闸门窗口 x∈[1850,2060] 内 y=40..232 全部九档 4/4 全回卷。
#   ② 路标版训练确实教会了它往下走（过 x1900/2040 时 y 从 191-229 降到 127，路标稳定吃到），
#      照样在 x≈2050 回卷。
#   ⇒ 判据是**更早那一段走的哪条道**（一个路径状态），不是"到了闸门站在哪"（一个位置状态）。
#   这正是"查到正解也没用"的原因：正解讲的是**走哪条路**，我一直把它编码成**终点站在哪**。
#   下次要解，得先找到那个路径状态在 RAM 里的位置，或者直接做真正的 Go-Explore（存状态而非存位置）。
