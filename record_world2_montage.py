"""World2 成果 montage：让 WideCNN student 依次打 2-1/2-2/2-3/2-4，各录一个通关局拼成一条 GIF。"""
...
MODEL = sys.argv[1] if len(sys.argv) > 1 else "mario_w2_wide.zip"
model = PPO.load(MODEL, device="cpu")

montage = []
for stage in ["2-1", "2-2", "2-3", "2-4"]:
...
imageio.mimsave("mario_world2.gif", montage, fps=20)
print(f">>> mario_world2.gif  共 {len(montage)} 帧")
